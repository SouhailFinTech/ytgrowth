...
FIX: Sidebar cannot be hidden (Collapse button removed via CSS)
...
def db_save(table: str, data: dict, user_id: str = None):
    ...
    if not sb:
        ...
        if isinstance(st.session_state[table], list): st.session_state[table].append(data)
        return True
    try:
...
def db_fetch(table: str, user_id: str = None, limit: int = 50):
    sb = get_supabase()
    if not sb: return st.session_state.get(table, [])
    try:
        q = sb.table(table).select("*").order("created_at", desc=True).limit(limit)
        if user_id: q = q.eq("user_id", user_id)
        return q.execute().data
    except Exception: return st.session_state.get(table, [])
...
def get_google_auth_url():
    ...
    return f"https://accounts.google.com/o/oauth2/v2/auth?client_id={client_id}&redirect_uri={redirect}&response_type=code&scope=openid email profile&access_type=offline"

def exchange_code_for_token(code: str):
    try:
        resp = requests.post("https://oauth2.googleapis.com/token", data={
            "code": code, "client_id": st.secrets.get("GOOGLE_CLIENT_ID",""),
...
def get_user_id(): return st.session_state.get('user', {}).get('id', 'anonymous')

def login_page():
    st.markdown("""<div style='min-height:100vh;display:flex;align-items:center;justify-content:center;'>
    ...
    if "code" in params:
        with st.spinner("Signing you in..."):
            ...
            if token_data and "access_token" in token_data:
                ...
                if user_info:
                    st.session_state['user'] = {'id': user_info.get('id', 'anon'), 'email': user_info.get('email', ''), 'name': user_info.get('name', 'Creator'), 'picture': user_info.get('picture', ''), 'access_token': token_data.get('access_token', '')}
                    rows = db_fetch('user_configs', get_user_id(), 1)
                    if rows: st.session_state['config'] = rows[0].get('config_json', {})
                    st.query_params.clear()
                    st.rerun()
    if auth_url:
        st.markdown(f"""<a href="{auth_url}" style='display:inline-block;background:#00e5a0;color:#000;font-weight:700;padding:0.75rem 2rem;border-radius:10px;text-decoration:none;font-size:0.95rem;margin-bottom:1rem;'>🔐 Sign in with Google</a>""", unsafe_allow_html=True)
    else:
        if st.button("🚀 Continue as Demo User", use_container_width=True):
            st.session_state['user'] = {'id': 'demo', 'email': 'demo@algoquant.studio', 'name': 'Demo Creator', 'picture': '', 'access_token': ''}
            st.rerun()
...
def init_session():
    if 'config' not in st.session_state:
        st.session_state['config'] = {'channel_name': 'AlgoQuant Trading', 'creator_bio': 'Financial engineer from Morocco, self-taught quant', 'products': 'SaaS, MQL5 EAs, courses, freelance', 'subscribers': 5, 'watch_hours': 1.4, 'avg_ctr': 2.5, 'total_videos': 4, 'gemini_api_key': '', 'youtube_api_key': '', 'email': ''}
    try:
        if hasattr(st, 'secrets'):
            cfg = st.session_state['config']
            if 'GEMINI_API_KEY' in st.secrets and not cfg.get('gemini_api_key'): cfg['gemini_api_key'] = st.secrets['GEMINI_API_KEY']
            if 'YOUTUBE_API_KEY' in st.secrets and not cfg.get('youtube_api_key'): cfg['youtube_api_key'] = st.secrets['YOUTUBE_API_KEY']
    except Exception: pass

# ════════════════════════════════════════════════════════════
# PAGE CONFIG & CSS (WITH SIDEBAR FIX)
# ════════════════════════════════════════════════════════════

st.set_page_config(page_title="AlgoQuant Studio", page_icon="⚡", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap');
:root { --bg:#0a0c10;--surface:#111318;--border:#1e2229;--accent:#00e5a0;--accent2:#0066ff;--warn:#ff6b35;--text:#e8eaf0;--muted:#6b7280;--green:#00e5a0;--red:#ff4560;--yellow:#ffd700; }
...
/* ✅ FIX: Hide sidebar collapse button so it can't be hidden */
...
DEFAULT_CHANNELS = {
    'Part Time Larry': 'UCY2ifv8iH1Dsgjrz-h3lWLQ', 'The Quant Science': 'UCnMn36GT_H0X-w5_ckLtlgQ',
    'Algovibes': 'UCF5Whbu7E7OAK0RUljUKS8w', 'Quantra': 'UCbmNph6atAoGfqLoCL_duAg',
}
...
def call_gemini(model, prompt, max_tokens=2000):
    for attempt in range(2):
        try:
            resp = model.generate_content(prompt, generation_config=genai.types.GenerationConfig(max_output_tokens=max_tokens, temperature=0.7), request_options={"timeout": 120})
            raw = resp.text.strip()
...
def build_context(niche="📈 Trading/Quant"):
    CONTEXTS = {
        "📈 Trading/Quant": f"""Channel: {cfg.get('channel_name','AlgoQuant Trading')}
...
What works: Titles starting with I (100% success), Dollar sign (113k avg views), Python/backtest/FTMO/algo/bot, Honest failure content.
Stats: {cfg.get('subscribers',5)} subs · {cfg.get('avg_ctr',2.5)}% CTR · {cfg.get('watch_hours',1.4)}h
Hook rules: First sentence MUST contain result or bold claim. NEVER start with Hi, Welcome. TTS-friendly short sentences.""",
        " Comedy/Entertainment": f"""Channel: Comedy/Entertainment Channel
...
What works: Relatable pain points, Absurd comparisons, Self-mockery, Trending topics/memes, Short punchy hooks.
Stats: {cfg.get('subscribers',5)} subs · {cfg.get('avg_ctr',2.5)}% CTR
Hook rules: First 3 seconds = punchline or bold statement. NEVER explain setup first. Pattern interrupt immediately. Keep it PG-13, shareable.""",
        "🏃 Sports/Fitness": f"""Channel: Sports/Fitness Content
...
What works: Before/after transformations, Myth-busting common beliefs, Pro athlete analysis, Controversial takes (respectfully).
Hook rules: Lead with shocking stat or result. Challenge conventional wisdom. Show don't tell.""",
        "💼 Business/SaaS": f"""Channel: Business/SaaS Content
...
What works: Revenue breakdowns, "How I built X in Y days", Failure stories with lessons, Tool recommendations.
Hook rules: Lead with specific numbers. Promise actionable value. Show proof/results.""",
        "🎮 Gaming": f"""Channel: Gaming Content
...
What works: Tier lists, "I played X for 100 hours", Hidden secrets/easter eggs, Competitive analysis.
Hook rules: Start with shocking gameplay moment. Promise exclusive info. High energy, fast pace.""",
        " Tech/Programming": f"""Channel: Tech/Programming Content
...
What works: "Build X in Y minutes", Tool comparisons, Debugging horror stories, Career advice.
Hook rules: Show the end result first. Promise specific skill gain. Keep it practical.""",
        "🎬 General/Other": f"""Channel: General Content Creator
...
Hook rules: Curiosity gap in first sentence. Bold claim or surprising fact. Promise clear value. Keep it tight, no fluff."""
    }
...
def fetch_competitor_videos(api_key, channels, n=20):
    ...
    for name, cid in channels.items():
        try:
            ...
            vids, npt = [], None
            while len(vids) < n:
                pr = yt.playlistItems().list(part='contentDetails',playlistId=uid,maxResults=min(50,n-len(vids)),pageToken=npt).execute()
                for it in pr['items']: vids.append(it['contentDetails']['videoId'])
                npt = pr.get('nextPageToken')
                if not npt: break
            for i in range(0,len(vids),50):
                sr = yt.videos().list(part='snippet,statistics,contentDetails',id=','.join(vids[i:i+50])).execute()
                for v in sr['items']:
                    ...
                    out.append({'channel':name,'channel_subs':subs,'title':sn['title'],'video_id':v['id'],'url':f"https://youtube.com/watch?v={v['id']}",'published':pub.strftime('%Y-%m-%d'),'days_old':dold,'views':views,'likes':int(st2.get('likeCount',0)),'comments':int(st2.get('commentCount',0)),'views_per_day':round(views/dold,1),'duration_sec':tsec,'is_short':tsec<=60})
        except Exception: continue
    return out

def analyze_patterns(videos, threshold=5000):
    STOP = {'a','an','the','and','or','but','in','on','at','to','for','of','with','by','from','is','it','my','i','your','how','what','why','when','this','that','you','we','are','was','be','have','has','do','did','will','can','get','as','its','not','into','using','vs','if','so'}
    ...
    return {'total':len(videos),'long_count':len(lf),'short_count':len(sh),'success_count':len(succ),'top_words':top_words,'trending':trending,'evergreen':evergreen,'top20':top20}

def ai_virality(model,idea,fmt,funnel,ctx):
    return call_gemini(model,f"""You are a YouTube growth expert.\n{ctx}\nIdea: {idea}\nFormat: {FORMAT_CONTEXT[fmt]}\nFunnel: {funnel}\nScore 0-100 across audience_demand,trend_alignment,differentiation,creator_fit,monetization_fit (each 20).\nIf below 60 suggest better angle. If 60+ approve.\nReturn ONLY valid JSON no markdown:\n{{"idea_summary":"","virality_score":78,"breakdown":{{"audience_demand":16,"trend_alignment":15,"differentiation":14,"creator_fit":18,"monetization_fit":15}},"verdict":"approved","verdict_reason":"","better_angle":null}}""",800)

def ai_title_hook(model,idea,fmt,funnel,ctx):
    return call_gemini(model,f"""You are a YouTube growth expert.\n{ctx}\nIdea: {idea}\nFormat: {FORMAT_CONTEXT[fmt]}\nFunnel: {FUNNEL_DESCRIPTIONS[funnel]}\nBest title above 75 score and hook. Hook: result first sentence TTS-ready never Hi.\nShort under 130 words. Long 60-90 words.\nReturn ONLY valid JSON no markdown:\n{{"title":"","title_score":82,"title_reasoning":"","hook_script":"","hook_score":85,"hook_reasoning":"","alternative_titles":[{{"title":"","score":78}},{{"title":"","score":75}},{{"title":"","score":76}}]}}""",1200)

def ai_script_part(model, idea, title, hook, funnel, ctx, fmt, part, prev=''):
    cont = f'\nContinue from: "{prev[-300:]}"' if part == 2 else ''
    if fmt == 'short': inst = 'Write the COMPLETE SHORT script. STRICTLY 100-130 words total. Under 55 seconds when spoken. Fast pace. Result first. Zero fluff. End with direct CTA.'
    else: inst = 'Write FIRST HALF ~1000 words. End at natural transition.' if part == 1 else 'Write SECOND HALF ~1000 words. End with subscribe CTA then funnel CTA.'
    return call_gemini_text(model, f"""You are a YouTube scriptwriter.\n{ctx}\nFormat: {FORMAT_CONTEXT[fmt]}\nTitle: {title}\nIdea: {idea}\nFunnel: {FUNNEL_DESCRIPTIONS[funnel]}\n{cont}\nRules: TTS-friendly short sentences. Show concept not code. {'Include [minute] markers.' if fmt=='long' else 'Keep it tight, one continuous flow. No [minute] markers.'} Never Hi Welcome.\n{inst}\n{'Start with: '+hook if part==1 else ''}\nReturn ONLY raw script text. No JSON. No markdown.""", 800 if fmt=='short' else 2000)

def ai_packaging(model,idea,title,fmt,funnel,ctx):
    return call_gemini(model,f"""You are a YouTube packaging expert.\n{ctx}\nTitle: {title}\nIdea: {idea}\nFormat: {fmt}\nFunnel: {FUNNEL_DESCRIPTIONS[funnel]}\nGenerate thumbnail 2 options SEO 3 shorts CTA.\nReturn ONLY valid JSON no markdown:\n{{"thumbnail":{{"option_1":{{"concept":"","background":"","main_text":"","sub_text":"","visual":"","colors":["#hex"],"canva_steps":"","predicted_ctr":""}},"option_2":{{"concept":"","background":"","main_text":"","sub_text":"","visual":"","colors":["#hex"],"canva_steps":"","predicted_ctr":""}},"recommended":"1","recommended_reason":""}},"seo":{{"tags":["t1","t2","t3","t4","t5","t6","t7","t8","t9","t10"],"description_line1":"","description_line2":"","chapters":[{{"time":"0:00","title":""}},{{"time":"2:00","title":""}},{{"time":"5:00","title":""}},{{"time":"10:00","title":""}},{{"time":"14:00","title":""}}]}},"shorts":[{{"title":"","hook":"","clip":""}},{{"title":"","hook":"","clip":""}},{{"title":"","hook":"","clip":""}}],"cta_script":""}}""",2500)
...
def ai_suggestions(model,trending,existing,ctx):
    ...
    return call_gemini(model,f"""YouTube content strategist.\n{ctx}\nTrending:\n{tt}\nPosted (do not repeat):\n{et}\nSuggest 3 ideas: 1 Short+2 Long. Titles above 75. Hooks result-first no Hi.\nReturn ONLY valid JSON no markdown:\n{{"date":"{datetime.now().strftime('%Y-%m-%d')}","suggestions":[{{"id":1,"format":"short","topic":"","why_now":"","title":"","title_score":80,"hook":"","show":"","hide":"","funnel":"ea","cta":"","thumb_text":"","tags":["t1","t2","t3","t4","t5"]}},{{"id":2,"format":"long_form","topic":"","why_now":"","title":"","title_score":85,"hook":"","show":"","hide":"","funnel":"ea","cta":"","thumb_text":"","tags":["t1","t2","t3","t4","t5"]}},{{"id":3,"format":"long_form","topic":"","why_now":"","title":"","title_score":82,"hook":"","show":"","hide":"","funnel":"ea","cta":"","thumb_text":"","tags":["t1","t2","t3","t4","t5"]}}]}}""",3000)
...
def ai_extract_shorts_from_long(model, long_script, ctx, funnel='ea'):
    return call_gemini(model, f"""You are a YouTube Shorts strategist.\n{ctx}\nOriginal Long Script:\n{long_script[:4000]}\nExtract 3 completely different Shorts from this long script. Each Short must focus on a distinct concept, trap, or result from the original.\nRules:\n- Format: UNDER 60 seconds (100-130 words each)\n- Hook: First sentence MUST be the result/bold claim. Never Hi/Welcome.\n- Structure: Hook (0-3s) -> Value/Proof (3-40s) -> CTA (40-55s)\n- Include visual cues for each segment\n- Funnel: {funnel}\nReturn ONLY valid JSON no markdown:\n{{"shorts":[{{"title":"", "title_score":85, "hook":"", "script":"", "description":"", "tags":["t1","t2","t3"], "visual_plan":{{"hook_visual":"","value_visual":"","cta_visual":""}}, "why_it_works":""}},{{"title":"", "title_score":82, "hook":"", "script":"", "description":"", "tags":["t1","t2","t3"], "visual_plan":{{"hook_visual":"","value_visual":"","cta_visual":""}}, "why_it_works":""}},{{"title":"", "title_score":80, "hook":"", "script":"", "description":"", "tags":["t1","t2","t3"], "visual_plan":{{"hook_visual":"","value_visual":"","cta_visual":""}}, "why_it_works":""}}]}}""", 3500)
...
def send_email_report(report_data, email):
    try:
        ...
        html = f"""<div style='font-family:sans-serif;max-width:600px;margin:0 auto;background:#111318;color:#e8eaf0;padding:2rem;border-radius:12px;'>
            <h1 style='color:#00e5a0;'>⚡ AlgoQuant Weekly Report</h1><p style='color:#6b7280;'>{report_data.get('report_date','')}</p>
            ...
            <h2 style='color:#e8eaf0;'>Diagnostics</h2><p><b>CTR:</b> {report_data.get('ctr_diagnosis','')}</p><p><b>Retention:</b> {report_data.get('retention_diagnosis','')}</p>
            <hr style='border-color:#1e2229;'><p style='color:#6b7280;font-size:0.75rem;'>AlgoQuant Studio</p></div>"""
        resp = requests.post("https://api.resend.com/emails", headers={"Authorization": f"Bearer {resend_key}", "Content-Type": "application/json"}, json={"from": "AlgoQuant Studio <report@algoquant.studio>", "to": [email], "subject": f"⚡ Weekly Content Report — {report_data.get('report_date','')}", "html": html})
...
def video_card_html(v):
    ...
    st.markdown(f"""<div class='video-card'><div style='display:flex;justify-content:space-between;align-items:flex-start;'>
        <div style='flex:1;'><div style='font-size:0.85rem;font-weight:600;margin-bottom:6px;'>{v['title']}</div>
...
with st.sidebar:
    ...
    if pic: st.markdown(f"<div style='display:flex;align-items:center;gap:0.75rem;padding:0.75rem 0 1.25rem;'><img src='{pic}' style='width:36px;height:36px;border-radius:50%;border:2px solid #00e5a0;'><div><div style='font-size:0.85rem;font-weight:600;'>{name}</div><div style='font-size:0.7rem;color:#6b7280;'>{user.get('email','')}</div></div></div>", unsafe_allow_html=True)
    else: st.markdown(f"<div style='padding:0.75rem 0 1.25rem;'><div style='font-size:1.4rem;font-weight:700;color:#00e5a0;'>⚡ AlgoQuant</div><div style='font-size:0.72rem;color:#6b7280;text-transform:uppercase;letter-spacing:0.08em;'>Content Intelligence Studio</div></div>", unsafe_allow_html=True)

    page = st.radio("", ["🏠  Dashboard","🔍  Competitor Intel","📊  My Channel", "🏭  Video Factory","📁  History","📧  Weekly Report","⚙️  Settings"], label_visibility="collapsed")
...
def page_dashboard():
    ...
    items = [(str(subs),"Subscribers",f"{sp:.1f}% of 1,000",'#00e5a0'), (f"{hours:.1f}h","Watch Hours",f"{hp:.3f}% of 4,000h",'#0066ff'), (f"{cfg.get('avg_ctr',2.5)}%","Avg CTR","Target: 4%+",'#ffd700' if cfg.get('avg_ctr',2.5)<4 else '#00e5a0'), (str(cfg.get('total_videos',4)),"Videos Posted","Keep posting",'#00e5a0')]
...
def page_factory():
    ...
    if not gemini_key: st.warning("️  Add your Gemini API key in Settings."); return
    ...
    with tab1:
        ...
        with col1: idea = st.text_area("Idea", placeholder="e.g. Build a prop firm EA... or Why women say 'we need to talk' comedy bit", height=80, label_visibility='collapsed')
        with col2: niche = st.selectbox("Niche", ["📈 Trading/Quant", " Comedy/Entertainment", "🏃 Sports/Fitness", "💼 Business/SaaS", " Gaming", "🔧 Tech/Programming", "🎬 General/Other"], key="factory_niche")
        with col3: fmt = st.selectbox("Format", ["long", "short"], key="factory_fmt")
        with col4: funnel = st.selectbox("Funnel", list(FUNNEL_DESCRIPTIONS.keys()), key="factory_funnel")
        if st.button("⚡  Run Video Factory",use_container_width=True) and idea.strip():
            model=get_model(); ctx=build_context(niche)
            st.markdown("<hr class='divider'>",unsafe_allow_html=True)
            ...
            th_data=pk.get('thumbnail',{}); seo=pk.get('seo',{}); shorts=pk.get('shorts',[]); cta=pk.get('cta_script',''); rec=th_data.get('recommended','1')
            tc1, tc2 = st.columns(2)
            for col, key in zip([tc1, tc2], ['option_1', 'option_2']):
                with col:
                    opt = th_data.get(key, {}); num = key.split('_')[1]; is_rec = rec == num; border = '#00e5a0' if is_rec else '#1e2229'; rec_tag = ' ⭐ RECOMMENDED' if is_rec else ''
    ...
    with tab2:
        ...
        if st.button("💡  Generate 3 Video Ideas",use_container_width=True):
            ...
            if not trending: trending=[{'title':'AI trading bot Python','views_per_day':1200,'days_old':5},{'title':'FTMO prop firm algo','views_per_day':800,'days_old':10}]; st.info("Using generic trends. Run Competitor Intel for better suggestions.")
            with st.spinner("Generating..."):
                try: result=ai_suggestions(model,trending,existing,ctx)
                except Exception as e: st.error(str(e)); return
            for s in result.get('suggestions',[]):
                fmt_=s.get('format','').upper().replace('_',' '); score=s.get('title_score',0); score_class = 'score-green' if score>=75 else 'score-yellow'; fmt_c='#0066ff' if 'LONG' in fmt_ else '#00e5a0'; tags_html=''.join([f"<span class='tag'>{t}</span>" for t in s.get('tags',[])])
                card_html = f"<div class='video-card' style='margin-bottom:1rem;'><div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;'><div><span style='font-size:0.65rem;font-weight:700;color:{fmt_c};border:1px solid {fmt_c};border-radius:4px;padding:1px 6px;margin-right:6px;'>IDEA #{s.get('id')} · {fmt_}</span><span class='score-badge {score_class}' style='font-size:0.65rem;'>Title {score}/100</span></div><span class='funnel-badge'>{s.get('funnel','').upper()}</span></div><div style='font-size:0.9rem;font-weight:700;margin-bottom:4px;'>\"{s.get('title','')}\"</div><div style='font-size:0.78rem;color:#6b7280;margin-bottom:8px;'>{s.get('topic','')} · {s.get('why_now','')}</div><div style='font-size:0.75rem;color:#9ca3af;background:#0d1117;border-radius:6px;padding:0.6rem 0.8rem;margin-bottom:8px;font-family:JetBrains Mono,monospace;'>{s.get('hook','')}</div><div style='font-size:0.72rem;color:#6b7280;margin-bottom:4px;'>📺 {s.get('show','')}</div><div style='font-size:0.72rem;color:#6b7280;margin-bottom:4px;'>🔒 {s.get('hide','')}</div><div style='font-size:0.72rem;color:#6b7280;margin-bottom:8px;'> {s.get('cta','')}</div><div>{tags_html}</div></div>"
                st.markdown(card_html, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if st.button("📊  Score Title & Hook",use_container_width=True) and score_title.strip():
            ...
            with c1_m: title_emoji = '🟢' if ts>=75 else '' if ts>=50 else ''; st.markdown(f"<div class='metric-card'><div class='metric-val'>{title_emoji} {ts}/100</div><div class='metric-lbl'>Title Score</div></div>",unsafe_allow_html=True)
            with c2_m: hook_emoji = '🟢' if hs>=75 else '🟡' if hs>=50 else '🔴'; st.markdown(f"<div class='metric-card'><div class='metric-val'>{hook_emoji} {hs}/100</div><div class='metric-lbl'>Hook Score</div></div>",unsafe_allow_html=True)
            step_box("Title Diagnosis",result.get('title_diagnosis','')); step_box("Hook Diagnosis",result.get('hook_diagnosis',''))
            ...
            if result.get('hook_rewrites'):
                best_v=result.get('best_hook_version','A'); section("Hook Rewrites")
                for hw in result['hook_rewrites']:
                    is_best=hw['version']==best_v; border='#00e5a0' if is_best else '#1e2229'; label=" ⭐ USE THIS" if is_best else ""
    ...
    with tab4:
        ...
        if brief_btn and th_title:
            ...
            st.markdown(f"""<div class='metric-card' style='margin-top:1rem;'><div style='font-size:0.72rem;color:#6b7280;margin-bottom:12px;text-transform:uppercase;letter-spacing:0.08em;'>Thumbnail Design Brief</div><div style='font-size:1.1rem;font-weight:700;margin-bottom:4px;'>Main text: "{th_brief.get('main_text','')}"</div>{sub_text_brief}<div style='font-size:0.78rem;color:#9ca3af;margin-bottom:8px;'>Layout: {th_brief.get('layout','')}</div><div style='margin-bottom:12px;'>{colors_html}</div><div style='font-size:0.75rem;color:#ffd700;margin-bottom:8px;'>CTR target: {th_brief.get('predicted_ctr','')}</div><div style='font-size:0.78rem;font-weight:600;margin-bottom:6px;'>Canva steps:</div><div style='font-size:0.75rem;color:#9ca3af;'>{th_brief.get('canva_steps','')}</div></div>""", unsafe_allow_html=True)
            st.session_state['th_brief'] = th_brief
        if gen_btn and th_title:
            brief=st.session_state.get('th_brief',{}); img_prompt=brief.get('image_prompt','') if brief else f"YouTube thumbnail dark background {th_title} {th_result_text} bold text high contrast professional trading"
            if not img_prompt: img_prompt=f"YouTube thumbnail for algorithmic trading video titled {th_title}, dark background, bold text, high contrast, professional, {th_style}"
            with st.spinner("Generating thumbnail image... (free via Pollinations AI, takes 15-20 seconds)"):
                ...
                else: st.error("Image generation failed. Try the design brief instead and build in Canva manually.")
    with tab5:
        ...
        with col_b:
            if st.button("⚡  Extract 3 Shorts", use_container_width=True) and long_script_input.strip():
                ...
                with st.spinner("Analyzing script & extracting Shorts..."):
                    try: result = ai_extract_shorts_from_long(model, long_script_input, ctx, ext_funnel)
                    except Exception as e: st.error(f"Extraction failed: {e}"); return
                if 'shorts' in result:
                    for i, s in enumerate(result['shorts'], 1):
                        ...
                        with c2:
                            ...
                            st.markdown(f"""<div class='metric-card'><div class='metric-lbl'>🎥 VISUAL PLAN</div><div style='margin-top:8px;font-size:0.78rem;color:#9ca3af;'><div style='margin-bottom:6px;'><b style='color:#00e5a0;'>0-3s (Hook):</b> {vp.get('hook_visual','Show result/chart spike')}</div><div style='margin-bottom:6px;'><b style='color:#0066ff;'>3-40s (Value):</b> {vp.get('value_visual','Screen recording/code walkthrough')}</div><div><b style='color:#ffd700;'>40-55s (CTA):</b> {vp.get('cta_visual','Point to description/link overlay')}</div></div></div>""", unsafe_allow_html=True)
                        ...
                        with col_dl: st.download_button(f"️ Download Short #{i}", s.get('script',''), file_name=f"short_{i}_{datetime.now().strftime('%Y%m%d')}.txt", key=f"dl_short_{i}")
            ...
            elif not long_script_input.strip(): st.info("👉 Paste your long script above to start extraction.")
...
def page_history():
    ...
    for r in records:
        ts_=r.get('title_score',0); hs_=r.get('hook_score',0); vs_=r.get('virality_score',0); real_ctr=r.get('real_ctr'); real_ret=r.get('real_retention'); created=r.get('created_at','')[:10] if r.get('created_at') else ''
...
def page_weekly_report():
    ...
    with st.form("email_setup"):
        ...
        if st.form_submit_button("💾  Save Email"):
            st.session_state['config']['email']=email_input; db_save('user_configs',{'config_json':st.session_state['config']},user_id); st.success("✅  Email saved")
    st.markdown("<hr class='divider'>",unsafe_allow_html=True)
    ...
    if gen_report:
        model=get_model(); competitor_data=st.session_state.get('competitor_data',{})
        if not competitor_data: st.info("No competitor data found. Run Competitor Intel first for better report quality."); competitor_data={'trending':[{'title':'FTMO prop firm algo','views_per_day':800,'days_old':5}]}
        ...
        st.markdown(f"""<div style='background:rgba(0,229,160,0.05);border:1px solid rgba(0,229,160,0.2);border-radius:12px;padding:1.5rem;margin-top:1rem;'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;'><div style='font-size:1rem;font-weight:700;color:#00e5a0;'> Weekly Content Report</div><div style='font-size:0.75rem;color:#6b7280;'>{report.get('report_date','')}</div></div><div style='font-size:0.85rem;color:#9ca3af;margin-bottom:1.5rem;'>{report.get('weekly_summary','')}</div></div>""", unsafe_allow_html=True)
        ...
        for idea in report.get('top_3_video_ideas',[]):
            ...
            st.markdown(f"""<div class='video-card'><div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;'><div style='font-size:0.9rem;font-weight:700;'>#{idea.get('rank')} {idea.get('title','')}</div><span style='font-size:0.7rem;font-weight:700;color:{score_c};border:1px solid {score_c};border-radius:4px;padding:1px 6px;'>Virality {idea.get('estimated_virality',0)}</span></div><div style='font-size:0.78rem;color:#9ca3af;font-family:JetBrains Mono,monospace;margin-bottom:6px;'>{idea.get('hook','')}</div><div style='font-size:0.72rem;color:#6b7280;'>{idea.get('why_this_week','')} · Funnel: {idea.get('funnel','')}</div></div>""", unsafe_allow_html=True)
        section("This Week Priorities")
        for p in report.get('this_week_priorities',[]): step_box("→",p)
        section("Diagnostics")
        step_box("CTR",report.get('ctr_diagnosis','')); step_box("Retention",report.get('retention_diagnosis','')); step_box("Thumbnail Fix",report.get('thumbnail_fix',''),'#ffd700')
        db_save('weekly_reports',{'user_id':user_id,'report':json.dumps(report)},user_id); st.success("✅  Report saved to history")
    if send_report:
        report=st.session_state.get('weekly_report')
        if not report: st.warning("Generate the report first, then send.")
        else:
            resend_key=st.secrets.get("RESEND_API_KEY","") if hasattr(st,'secrets') else ""
            if not resend_key: st.info("RESEND_API_KEY not configured in Streamlit secrets. Add it to enable email sending.")
            else:
                with st.spinner("Sending email..."): ok=send_email_report(report,cfg.get('email',''))
...
def page_settings():
    ...
    with st.form("settings"):
        ...
        if st.form_submit_button("💾  Save All Settings",use_container_width=True):
            new_config={'gemini_api_key':gem_key,'youtube_api_key':yt_key,'channel_name':ch_name,'creator_bio':bio,'products':prods,'email':email,'subscribers':new_s,'watch_hours':new_h,'avg_ctr':new_c,'total_videos':new_v}
            st.session_state['config']=new_config; db_save('user_configs',{'config_json':new_config},user_id); st.success("✅  Settings saved")
    st.markdown("<hr class='divider'>",unsafe_allow_html=True)
...
""", language="toml")
    ...
    st.markdown("""<div class='step-box'><div style='font-size:0.82rem;font-weight:600;margin-bottom:3px;'>1. Create free account at supabase.com</div><div style='font-size:0.75rem;color:#9ca3af;'>New project → copy URL and anon key from Settings → API</div></div><div class='step-box'><div style='font-size:0.82rem;font-weight:600;margin-bottom:3px;'>2. Run this SQL in Supabase SQL Editor</div></div>""", unsafe_allow_html=True)
    st.code("""-- Run this in Supabase SQL Editor
create table video_history (id uuid default gen_random_uuid() primary key, user_id text, idea text, format text, funnel text, niche text, title text, title_score int, hook_score int, virality_score int, word_count int, script text, tags text, status text default 'generated', real_ctr float, real_retention float, real_views int, created_at timestamp default now());
create table competitor_reports (id uuid default gen_random_uuid() primary key, user_id text, data text, channel_count int, created_at timestamp default now());
create table channel_snapshots (id uuid default gen_random_uuid() primary key, user_id text, subscribers int, watch_hours float, avg_ctr float, total_videos int, created_at timestamp default now());
create table weekly_reports (id uuid default gen_random_uuid() primary key, user_id text, report text, created_at timestamp default now());
create table user_configs (id uuid default gen_random_uuid() primary key, user_id text unique, config_json jsonb, created_at timestamp default now());
""", language="sql")
